...
class CommunityCardDetector:
    ...
    def detect_community_cards(self, detections: list[dict]) -> list[dict]:
        ...
        Returns:
            List[Dict]: Processed community cards with assigned coordinates,
                or an empty list if no valid configuration is found.
        """
        if len(detections) < 3:
            return []
    ...
    def _group_cards_into_rows(self, detections: list[dict]) -> list[list[dict]]:
        ...
        for group in vertical_groups:
            ...
            for detection, center in group[1:]:
                _prev_detection, prev_center = current_row[-1]
    ...
    def _determine_configuration(
        self, rows: list[list[dict]], all_detections: list[dict]
    ) -> tuple[BoardConfiguration, dict | None]:
    ...
    def _find_chihuahua_card(self, rows: list[list[dict]], all_detections: list[dict]) -> dict | None:
            ...
            if self._is_between_rows(card, rows[:2]):
                ...
                if self._is_to_the_right(card, rows[:2]):
                    ...
                    if all(len(row) in (4, 5) for row in rows[:2]) and len(rows[0]) == len(rows[1]):
                        logger.debug(
                            f"Card {card['card']} passed main rows length check. Identified as Chihuahua card."
                        )
                        return card
    ...
    def _assign_coordinates(
        self, rows: list[list[dict]], configuration: BoardConfiguration, chihuahua_card: dict | None
    ) -> list[dict]:
        ...
        for y, row in enumerate(rows[:2], start=1):
            ...
            for x, card in enumerate(sorted_row, start=1):
                ...
                community_cards.append(
                    {"card": card["card"], "x": x_coord, "y": y_coord, "confidence": card.get("confidence", 1.0)}
                )

        # Chihuahua card always gets x=5 and y=2
        if configuration == BoardConfiguration.CHIHUAHUA and chihuahua_card:
            community_cards.append(
                {"card": chihuahua_card["card"], "x": 5, "y": 2, "confidence": chihuahua_card.get("confidence", 1.0)}
            )
