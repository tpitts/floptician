...
class FFmpegCameraCapture:
    ...
    def stop(self):
        ...
        if self.proc:
            try:
                self.proc.terminate(); self.proc.wait(0.5)
            except:
                self.proc.kill()
                ...
                if video:
                    ...
                    if m:
                        self.cameras.append({
                            ...
                            'camera_name': m.group(2)
                        })
        elif is_windows() and windows_dshow:
            graph = FilterGraph(); devs = graph.get_input_devices()
            self.cameras = [{'camera_index': i, 'camera_name': n}
                            for i, n in enumerate(devs)]
        else:
            for i in range(5):
                cap = cv2.VideoCapture(i)
                if cap.isOpened() and cap.read()[0]:
                    self.cameras.append({'camera_index': i,
...
# Utility
list_all_cameras = lambda: CameraManager().get_available_cameras()
